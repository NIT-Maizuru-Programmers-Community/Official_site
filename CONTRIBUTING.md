...
| Ruleset                | 内容                                                                     | 迂回権限                           |
| ---------------------- | ------------------------------------------------------------------------ | ---------------------------------- |
| `main-base-protection` | PR必須、未解決レビュースレッドの解決必須、force push・ブランチ削除の禁止 | 管理者を含めなし                   |
...
2. 未解決のレビュースレッドを解決します。必須CIが導入された後は、その成功も必要です。
...
現在の設定専用PRでは、アプリ本体とCIを導入しません。存在しないチェックでマージが止まらないよう、必須ステータスチェックも未設定です。

MVPのCI導入後、`validate` の実行成功を確認してから、管理者が `main-base-protection` に必須ステータスチェック `validate` を追加します。チェックの提供元はGitHub Actionsを指定し、ベースブランチの最新状態に対する成功を必須にしてください。管理者もこのチェックは迂回しません。
