...
2. 最新の `main` から作業ブランチを作ります。名前は以下の命名ルールに従い、1ブランチで1つの目的を扱います。
...
人もCodex・Claudeも `種類/担当者のGitHub名/概要` に統一します。
...
複数の種類にまたがる場合は主目的で選びます。担当者にはGitHubユーザー名を使い、AIが作業する場合も依頼者の名前を入れます。担当者が不明なら確認し、Git認証中のアカウントから推測しません。`codex/`・`claude/`の特別扱いはありません。

概要は半角英小文字・数字・ハイフンで短く記述します。関連Issueがあれば概要の先頭に番号を付け、なければ省略します。

- 機能追加：`feature/HizKz/123-add-news`
- 不具合修正：`fix/HizKz/124-mobile-menu`
- Issueのない文書変更：`docs/HizKz/contribution-guide`

`HizKz` とIssue番号は自分のユーザー名・実際の番号へ置き換えてください。同名ブランチがあれば概要を具体化し、既存ブランチを流用・上書きしません。既存の作業ブランチは改名不要で、新しく作るブランチから適用します。命名は文書上の共通ルールであり、CIでは強制しません。

## コミット・PRとIssue

コミットとPRタイトルは `種類: 日本語の短い要約` を基本にします。種類はブランチと同じ一覧から選び、機能追加も `feature` を使います。例：`docs: ブランチ命名規則を追加`。これは部内の表記ルールであり、Conventional Commitsの導入・強制ではありません。

PR本文の関連Issue欄には、マージで完了させるIssueを `Closes #123`、参考として関連付けるIssueを `Refs #123` と記載します。Issueがなければ「なし」とします。

レビューの修正は同じブランチへcommit・pushし、同じPRで確認します。Squash merge後の次の作業は最新mainから新しいブランチを作り、マージ済みブランチを使い続けません。
